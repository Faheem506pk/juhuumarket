    ...
    const fetchProduct = async () => {
      try {
        const response = await fetch(
          `http://localhost:8080/https://api.juhuu.app/v1/products/${productId}`
        );
        ...
        <div className="container">
          ...
          : 'Description not available'}
                ...
                <div className="col-md-6">
                  <h3>Highlights</h3>
                  <ul>
  {product.product.highlights[language] && product.product.highlights[language].length > 0 ? (
    product.product.highlights[language].map((highlight, index) => (
